...
//   guard(touched) (optional) vetoes the commit if it returns false (e.g. a protected path
//                  was touched) — nothing is committed or cherry-picked;
...
// { ok, committed, blocked?, touched[], error? }. Requires the current working tree to be
// clean (callers gate on workingTreeClean) so the cherry-pick applies without conflict.
export function applyInWorktree({ edit, validate, message, guard, cwd = process.cwd() }) {
  ...
  try {
    ...
    if (guard && !guard(touched)) return { ok: false, committed: false, blocked: true, touched }; // vetoed (e.g. protected path)
