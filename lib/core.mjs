...
// Reduce gate results to a fail-closed verdict: any required gate that did not
// pass blocks. Unknown/errored == not passed (fail closed).
export function gateVerdict(results = []) {
  const required = results.filter((r) => r.required !== false);
  const failures = required.filter((r) => r.pass !== true);
