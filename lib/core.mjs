...
// Build a `--flag value` reader over an argv array. An absent flag, a flag at the end with
// no value, OR an explicit EMPTY value all fall back to the default — empty is never a
// meaningful value for these flags, and treating it as one would make a numeric flag like
// `--rounds ""` parse to NaN and silently disable a cap. ("0" is truthy, so real zeros are
// preserved.) Shared by loop.mjs + goal.mjs (were two copies).
