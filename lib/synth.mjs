...
//
// STATUS: the deterministic acceptance half (this module) is wired into /goal as the
// optional `synth` gate (enable with a `"synth": { "candidates": "<path>" }` block in config;
// see goal.mjs). The GENERATION half — running a mutation engine and asking a model for
// candidate tests — is external: it deposits a candidates JSON that the gate then admits or
// rejects with no model judgment (see docs/ROADMAP.md).
